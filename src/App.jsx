...
export default function App() {
  ...
  // BH Payout Summary — renders one section (new or upsell)
  const BHSection = ({ sectionDeals, type }) => {
    ...
    const color=isUpsell?'#f59e0b':'#0ea5e9'
    const bgColor=isUpsell?'rgba(245,158,11,0.06)':'rgba(14,165,233,0.06)'
    const borderColor=isUpsell?'rgba(245,158,11,0.25)':'rgba(14,165,233,0.25)'
    const textColor=isUpsell?'#92400e':'#0369a1'
    return(
      <div style={{ marginBottom:24,borderRadius:10,overflow:'hidden',border:`0.5px solid ${borderColor}`,boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
        <div style={{ padding:'10px 16px',background:bgColor,borderBottom:`0.5px solid ${borderColor}`,display:'flex',justifyContent:'space-between',alignItems:'center' }}>
          <div style={{ display:'flex',alignItems:'center',gap:10 }}>
            ...
            <span style={{ color:textColor,fontSize:14,fontWeight:700 }}>{isUpsell?'Existing Client Upsell':'New Business'}</span>
            <span style={{ fontSize:11,color:'#64748b',fontWeight:400 }}>{isUpsell?'4% of monthly SLA · Once-off payment · No clawback':'8% of 12-month SLA · Split 50/50 · P1 month 2, P2 month 7'}</span>
          </div>
          <div style={{ fontSize:13,color:textColor,fontWeight:700 }}>Commission: {fmt(sectionComm)} · Paid: {fmt(sectionPaid)}</div>
        </div>
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%',borderCollapse:'collapse',background:'#fff' }}>
            <thead>
              <tr>
                {['Client','Month','Lic Total',isUpsell?'4% Comm':'8% Comm','Payout 1','Comm Pay Date','P1 Status','Client 1st Payment',
                  ...(isAdmin?['Finance Action']:[]),
                  ...(!isUpsell?['Payout 2','P2 Date','P2 Status',...(isAdmin?['Finance Action']:[])]:[]),
                  'Cancellation','Approvals'
                ].map((h,i)=><th key={i} style={{ ...th,background:i>=8&&!isUpsell?'rgba(99,102,241,0.06)':th.background }}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {sectionDeals.map(d=>{
